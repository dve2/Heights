...
class FixedCropTestDataset(object):

  def __init__(self, base_dataset, crop_h = 192, crop_w = 192):

    self.base_dataset = base_dataset
    self.crops = []

    # Iterate through the base_dataset to pre-calculate crop coordinates
    for img_idx in range(len(self.base_dataset)):

      # Get image dimensions
      im_mask, image, mask, meta = self.base_dataset[img_idx]
      img_w, img_h = image.shape[1], image.shape[2]

      # Generate sliding window crops without overlap
      for top_y in range(0, img_h - crop_h + 1, crop_h):
        for left_x in range(0, img_w - crop_w + 1, crop_w):
          bottom_y = top_y + crop_h
          right_x = left_x + crop_w
          self.crops.append((img_idx, top_y, left_x, bottom_y, right_x))

      # TODO add caching for speed-up

  def __len__(self):
    return len(self.crops)

  # Helper method to get the original image number for a given crop number
  def get_img_num(self, corp_num):
    return self.crops[corp_num][0]

  # Helper method to extract the crop from an image
  def get_crop(self, img_tensor, corp_num):
    # img_data: [H,W] or [C,H,W] Torch tensor
    _, top, left, bottom, right = self.crops[corp_num]
    ...
    if img_tensor.ndim == 3: # [C,H,W]
        ...
        raise ValueError(f"Unsupported tensor dimensions for cropping: {img_tensor.ndim}")


  def __getitem__(self, corp_num):
  # number of corp
    img_num = self.get_img_num(corp_num)
    im_mask, image, mask, meta = self.base_dataset[img_num]

    two_channel_crop = self.get_crop(im_mask, corp_num)
    image_crop = self.get_crop(image, corp_num)
    mask_crop = self.get_crop(mask, corp_num)

    return two_channel_crop, image_crop, mask_crop, meta
