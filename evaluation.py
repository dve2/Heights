...
"""
    Evaluation script for heights model. It predict heights on previously masked image.
    It also calculates confidence intervals for MSE and MAE metrics.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run model test on test dataset")
    parser.add_argument(
        "--data-dir",
        default="tests/data",
...
def main():
    ...
    ds = CustomDataset2chdm(args.data_dir, transform=test_transforms, target_transform=target_transform)
    ...
    lit_model1 = Lit.load_from_checkpoint(args.weights, weights_only=False)
